...
describe('HNSW Stress Testing', () => {
  ...
  it('should validate memory usage patterns', () => {
    ...
    console.log(`🗑️ Memory change after deleting 250 nodes: ${deleteMemoryDelta.toFixed(2)}MB`);
  ...
  it('should handle different distance metrics', () => {
    ...
    configs.forEach(({ space, name }) => {
      ...
      console.log(`📊 ${name} distance: ${results.length} results`);
...
describe('HNSW Performance Benchmarks', () => {
  it('should demonstrate performance scaling', () => {
    ...
    sizes.forEach(size => {
      ...
      console.log(`📊 Size: ${size}, Build: ${buildTime.toFixed(2)}ms, Search: ${searchTime.toFixed(3)}ms`);
